...
const Value = union(enum) {
    ...
    pub fn zerdeSerialize(self: Value, serializer: anytype) @TypeOf(serializer.*).Error!void {
        switch (self) {
            ...
            .bool => |v| try serializer.serializeBool(v),
            .int => |v| try serializer.serializeInt(v),
            .uint => |v| try serializer.serializeInt(v),
            .float => |v| try serializer.serializeFloat(v),
            .string => |v| try serializer.serializeString(v),
            .array => |values| {
                var array = try serializer.beginArray();
                for (values) |v| try v.zerdeSerialize(&array);
                try array.end();
            },
            .object => |fields| {
...
const Adapter = struct {
    ...
    pub fn fixture(comptime T: type, allocator: Allocator, input: []const u8) !T {
...
fn parseObject(allocator: Allocator, deserializer: anytype) @TypeOf(deserializer.*).Error!Value {
    ...
    var index: usize = 0;
    while (try access.nextKey(allocator)) |key| : (index += 1) fields[index] = .{ .key = key, .value = try access.nextValue(Value, allocator) };
